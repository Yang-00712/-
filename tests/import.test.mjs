...
test('bundled XLSX worker lists sheets and imports only selected B/C rows',()=>{
 ...
 vm.runInContext(fs.readFileSync(new URL('../import-worker.js',import.meta.url),'utf8'),context);
 ...
 scope.onmessage({data:{action:'open',buffer:bytes}});assert.equal(messages.at(-1).type,'sheets');assert.equal(messages.at(-1).sheets[0],'合成資料');
