...
export function importParamsView(plant,input,{esc,button}){
 ...
 return `<div class="card card-pad">${select}<form id="import-params-form"><div class="fields">${fields.map(([key,label])=>`<div class="field"><label for="param-${key}">${label}</label><input id="param-${key}" name="${key}" value="${esc(value[key])}" ${['first','count'].includes(key)?'inputmode="numeric" type="number"':'autocapitalize="characters"'} required></div>`).join('')}</div><label class="context-toggle"><input type="checkbox" name="reverse" ${value.reverse?'checked':''}>再反轉D／E來源</label><div class="foot-actions"><button type="submit" class="button primary">儲存參數</button>${button('恢復廠別預設','import-params-reset','light','','type="button"')}</div></form><p class="inline-hint">套用下次匯入。工作頁仍以完整名稱選取；既有卡夾原碼不變。</p></div>`;
}
