...
export function manualTimeView(job,state,{esc,button}){
  ...
  const verdict=row=>!row.target?'—':row.guide?.error?'待確認':row.first?'起點免間隔':row.deficit?'還差 '+formatDuration(row.deficit):row.excess?'超過 '+formatDuration(row.excess):'範圍內';
  ...
  html+=`<div class="manual-workbench ${selectedRows.length?'has-selection':''}"><div class="table-scroll manual-scroll" id="manual-list" tabindex="0" aria-label="${label}手動時間，左右與上下滑動"><table class="manual-table"><colgroup>${[34,132,48,108,68,78,120,240].map(width=>`<col style="width:${width}px">`).join('')}</colgroup><thead><tr><th>#</th><th>時間／間隔</th><th>&lt;80</th><th>規則差額</th><th>樓層</th><th>型式</th><th>設備</th><th>完整設備碼</th></tr></thead><tbody>${list.slice(0,limited).map(rowHtml).join('')}${limited<list.length?'<tr data-manual-more-sentinel><td colspan="8">滑動載入更多…</td></tr>':''}</tbody></table></div>`;
  if(selectedRows.length){
    const current=selectedRows[0],preview=!locked?previewManualDeletion(job,plan,period,[...state.selection]):null;
    const metrics=(row,title)=>`<div class="manual-metrics"><div><span>${row.first?'起點時間':title}</span><strong>${row.first?formatClock(row.time):formatDuration(row.interval)}</strong></div><div><span>規則差額</span>${difference(row)}</div></div>`;
    const detail=row=>`${row.target?`<b>元件 ${row.sourceIndex+1} · ${esc(row.target.equipment||'—')}</b><p class="manual-rule">${esc(row.target.floor??'—')}F · ${esc(row.target.form||'—')}　${row.first?'起點，不計間隔':'需 '+esc(range(row.guide))}</p>`:'<b>備用／已刪時刻</b>'}`;
