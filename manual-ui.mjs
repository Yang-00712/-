...
export function manualTimeView(job,state,{esc,button}){
  ...
  if(staged&&plan.phase==='am'){
    html+=`<p>先刪除不需要的時刻，再確認上午末筆。</p><div class="foot-actions">${button(cutRow?`上午到第 ${cutRow.sourceIndex+1} 筆 · 確認末筆`:'沒有可確認的元件','manual-confirm-am','primary','check',cutRow&&!state.busy?`data-index="${cutRow.index}"`:'disabled')}</div><p class="inline-hint">${cutRow?`末筆 ${formatClock(cutRow.time)}。點選較前的元件，可提早結束上午。`:''}</p>`;
  }else if(staged){
    html+=`<p>上午已確認到第 ${amCount} 筆${lastAm?' · '+formatClock(lastAm.time):''}。</p><div class="foot-actions">${button('重開上午','manual-reopen-am','light small','undo')}${pmCount&&plan.phase!=='pm'?button('編排下午','manual-period','primary small','clock','data-period="pm"'):''}</div>`;
  }else{
  ...
  const verdict=row=>!row.target?'—':row.guide?.error?'待確認':row.deficit?'還差 '+formatDuration(row.deficit):row.excess?'超過 '+formatDuration(row.excess):'範圍內';
  const difference=row=>`<span class="manual-difference ${row.deficit||row.excess||row.guide?.error?'off-range':'in-range'}">${esc(verdict(row))}</span>`;
  ...
  html+=`<div class="manual-live-g ${failed?'has-fail':''}">G 最低 ${minimum??'—'} 分 · ${job.settings.mode==='outdoor'?'廠外僅顯示':`未過 ${failed} 個窗口`}</div><p class="manual-help">${locked?'上午已確認；要修改請先重開上午。':state.rangeMode?'點第一筆，再點最後一筆。':'# 是元件編號。點選後可刪除時刻或修改間隔。'}</p>${state.feedback?`<p class="manual-feedback" role="status">${esc(state.feedback)}</p>`:''}`;
  ...
  html+=`<div class="table-scroll manual-scroll" id="manual-list" tabindex="0" aria-label="${label}手動時間，左右與上下滑動"><table class="manual-table"><thead><tr><th>#</th><th>時間／間隔</th><th>G分</th><th>規則差額</th><th>樓層</th><th>型式</th><th>設備</th><th>完整設備碼</th></tr></thead><tbody>${list.slice(0,limited).map(rowHtml).join('')}${limited<list.length?'<tr data-manual-more-sentinel><td colspan="8">滑動載入更多…</td></tr>':''}</tbody></table></div>`;
  if(selectedRows.length){
    const current=selectedRows[0],preview=!locked?previewManualDeletion(job,plan,period,[...state.selection]):null;
    const metrics=(row,title)=>`<div class="manual-metrics"><div><span>${title}</span><strong>${formatDuration(row.interval)}</strong></div><div><span>規則差額</span>${difference(row)}</div></div>`;
    const detail=row=>`${row.target?`<b>元件 ${row.sourceIndex+1} · ${esc(row.target.equipment||'—')}</b><p class="manual-rule">${esc(row.target.floor??'—')}F · ${esc(row.target.form||'—')}　需 ${esc(range(row.guide))}</p>`:'<b>備用／已刪時刻</b>'}`;
    const editable=!locked&&selectedRows.length===1&&!current.excluded&&current.target;
    html+=`<div class="manual-dock"><div class="manual-deletion-preview">${detail(current)}${current.excluded?'<p>已刪時刻，可按還原。</p>':metrics(current,'目前間隔')}${editable?`<details class="manual-interval-editor"><summary>修改間隔</summary><form id="manual-seconds-form" data-index="${current.index}" data-period="${period}"><label for="manual-seconds">改為 分.秒</label><input id="manual-seconds" name="seconds" type="text" inputmode="decimal" value="${formatDuration(current.interval).replace(':','.')}" required><button class="button light small" type="submit">套用間隔</button></form><small class="manual-edit-rule">1.34＝1分34秒；只移動這筆與${label}後續時刻。</small></details>`:''}`;
    if(preview?.count){
