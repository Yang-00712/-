...
`manual-time.mjs` 是獨立手動候選，沒有呼叫自動求解器。基礎＋Rand取整數秒，上午／下午共用固定原間隔；排除索引各自保存，刪除只跨過時刻並累加間隔，不移除元件。映射需完整足量、保持元件次序；UI依主設定重算首筆實際等待。`inspectManualSchedule` 重用逐列上下限、完整小組、樓層、返回及80窗核對，不修改手動時刻。`manualResult.ok` 表示成功套用，`rulesOk` 才是規則通過；不能混同。
